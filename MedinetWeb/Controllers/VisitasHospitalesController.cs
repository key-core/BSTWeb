...
namespace MedinetWeb.Controllers
{
    [Authorize]
    [SessionExpireFilter]
    public class VisitasHospitalesController : Controller
    {
        ...
        {
            var recordList = db.SP_MW_VisHospitalPorReg(cicloID).ToList()
        ...
        {
            var recordList = db.SP_MW_VisHospitalPorRegLin(cicloID, regionID).ToList()
        ...
        {
            var recordList = db.SP_MW_VisHospitalPorRegLinVis(cicloID, regionID, lineaID).ToList()
        ...
        [Authorize(Roles = "Administrador, Invitado, Ventas, Mercadeo")]
        [SessionExpireFilter]
        public ActionResult GetAllRegLinVisHos(short cicloID, short regionID, short lineaID, short visitadorID, [DataSourceRequest] DataSourceRequest request)
        {
            var recordList = db.SP_MW_VisHospitalPorRegLinVisHos(cicloID, regionID, lineaID, visitadorID).ToList()
                                .OrderBy(e => e.Hospital);
            return Json(recordList.ToDataSourceResult(request, record => new
            {
                record.ID_Ciclo,
                record.ID_Region,
