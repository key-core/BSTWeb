...
namespace MedinetWeb.Controllers
{
    [Authorize]
    [SessionExpireFilter]
    public class BajaMedicoController : Controller
    {
        //
        // GET: /Fichero/
        ...
        [Authorize(Roles = "Visitador Web")]
        [SessionExpireFilter]
        public JsonResult Delete(string REG)
        {
            int visitadorID = (Int32)Membership.GetUser().ProviderUserKey;
            try
            {
                db.SP_MWL_BajaMedico(visitadorID, REG);
                return Json(true);
            }
            catch (Exception e)
            {
        ...
        public ActionResult GetAll([DataSourceRequest] DataSourceRequest request)
        {
            ...
            int visitadorID = (Int32)Membership.GetUser().ProviderUserKey;
            var recordList = db.SP_MWL_GetFichero(visitadorID).ToList();
