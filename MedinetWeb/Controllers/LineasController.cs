...
namespace MedinetWeb.Controllers
{
    ...
    public class LineasController : Controller
    {
        ...
        public ActionResult GetAll([DataSourceRequest] DataSourceRequest request)
        {
            ...
            var lista = _database.Lineas.GetAll();  //_database.Registers.GetAll();
        ...
        public ActionResult Add([DataSourceRequest] DataSourceRequest request, MW_Lineas record)
        {
            // Test if Register object and modelstate is valid.
            if (record != null && ModelState.IsValid)
            {
                // Add Register to UoW.
                _database.Lineas.Add(record);
        ...
        [SessionExpireFilter]
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult Update([DataSourceRequest] DataSourceRequest request, MW_Lineas record)
        {
            // Test if Register object and modelstate is valid.
            if (record != null && ModelState.IsValid)
            {
                // Update Register to UoW.
                _database.Lineas.Update(record);
        ...
        [SessionExpireFilter]
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult Delete([DataSourceRequest] DataSourceRequest request, MW_Lineas record)
        {
            // Test if Register object and modelstate is valid.           
            if (record != null && ModelState.IsValid)
            {
                // Delete Register from UoW.
                _database.Lineas.Delete(record);
        ...
        {
            IEnumerable<SelectListItem> listRecord = db.MW_Lineas
        ...
        [Authorize]
        [SessionExpireFilter]
        public JsonResult GetLineas_ConDis(Int16 IDMarca)
        {
            IEnumerable<SelectListItem> listRecord = db.SP_MWS_Lineas_ConDis(IDMarca).ToList()
                .Where(c => c.BO_Activo == true)
                .OrderBy(c => c.TX_Linea)
                .AsEnumerable().Select(c => new SelectListItem()
                {
                    Text = c.TX_Linea,
                    Value = c.ID_Linea.ToString()
